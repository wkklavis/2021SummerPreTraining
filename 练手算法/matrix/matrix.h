...
class Matrix {//������
 public:
  typedef T value_type;/**������Ԫ�ص��������� */ 
  typedef T& reference;/** ������Ԫ�ص������������� */
  typedef const T& const_reference;/** ������Ԫ�صĳ������������� */
  typedef T* pointer;/** ������Ԫ�ص�ָ���������� */
 ...
 public:
  ...
  // ==== Observers ====
 /* ��ȡ����Ԫ�ص�ֵ*/
  size_type row_size() const { return data_.size(); }/** ��ȡ�ж����� */

  size_type column_size() const { return column_size_; }/** ��ȡ�ж����� */

  size_type size() const {
    return row_size() * column_size();
  }

  std::pair<size_type, size_type> dimension() const {/**��ȡ�������״(����ά��) *//**����Python��numpy��shape�ṹ */
  ...
  reference operator()(size_type row, size_type column) {
    matrix::CheckIndicesRange(*this, row, column);/**�����������ֵ�Ƿ񳬳���Χ������������׳��쳣 */
    return data_[row][column];
  }

  const_reference operator()(size_type row, size_type column) const {/** ��ͨ���ã�������Ϊ��ֵ��Ҳ������Ϊ��ֵ */
  ...
  /**���������أ���������ָ�����ʽʵ�ֵģ���Ŀ����ֻ��ʵ��һ��"[]"�����أ���һ��"[]"
  * ������ͨ��ָ��ȡֵʵ�ֵģ���������Ͳ�������е�����ֵ�����жϣ���ע��
  */
  ...
  Matrix& operator*=(const value_type& scaler) {/** multiply */
    matrix::ConcurrentProcess(
  ...
  Matrix& operator/=(const value_type& scaler) {/** div */
    matrix::ConcurrentProcess(/**�������롰���š��ĳ˷����ƣ�ע������û���ж�scaler�Ƿ�Ϊ�㣬Ϊ�㽫�Զ��׳������쳣 */
        row_size(), [this, &scaler](size_type row) mutable {
          for (size_type column = 0; column < this->column_size(); ++column) {
            data_[row][column] /= scaler;
          }
  ...
  Matrix& operator%=(const value_type& scaler) {//ȡ�ำֵ/** mod */
    matrix::ConcurrentProcess(
        row_size(), [this, &scaler](size_type row) mutable {/** �����ÿ��Ԫ�����α�scalerȡ�� */
          for (size_type column = 0; column < this->column_size(); ++column) {
            data_[row][column] %= scaler;
          }
        });
  ...
  Matrix& operator*=(const Matrix& other) {/** ���ڻ��ĳ˷� */
    matrix::CheckDimensionMultipliable(*this, other);/**������������Ƿ�����˷����� */
  ...
  Matrix& elementary_row_switch(size_type row1, size_type row2) {//��ת��
  ...
  Matrix& elementary_row_multiply(                              //�г˻�
  ...
  Matrix& elementary_column_switch(size_type column1, size_type column2) {//��ת��
  ...
  // NOTE(clangpp): time complexity O(row_size()), space complexity O(1)
  Matrix& elementary_column_multiply(                                      //�г˻�
    ...
    for (size_type row = 0; row < row_size(); ++row) {
      futures[row] = std::async(
          ...
          [this, &other, row, &value_equal_to]() {
            for (size_type column = 0; column < this->column_size(); ++column) {
              if (!value_equal_to((*this)[row][column], other[row][column])) {/**ʹ�ô���ĶԱȺ������Ƚ��������������ͬ���д���Ԫ���Ƿ���� */
                return false;
              }
            }
            return true;
...
template <typename T>
Matrix<T> operator-(const Matrix<T>& lhs, Matrix<T>&& rhs) {
  matrix::CheckDimensionMatches(lhs, rhs);//����ƥ��ſ������
  Matrix<T> result(std::move(rhs));
  typedef typename Matrix<T>::size_type size_type;
  matrix::ConcurrentProcess(
      result.row_size(),
